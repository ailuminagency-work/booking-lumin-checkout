...
const fail=()=>{throw Error('RUNTIME_ARTIFACT_COLLECTION_FAILED');};
export async function collectRuntimeArtifacts(workspace) {
  ...
  for(const layout of ['public','extensions']) {
    const path=resolve(cache,'mode-runtime-browser-'+layout+'.json');
    let before;
    ...
    let approved;
    try{approved=await validateRuntimeArtifacts(root,receipt.artifactRunId);}
...
async function main(){
  try{
    const files=await collectRuntimeArtifacts(process.cwd());
    if(process.env.GITHUB_OUTPUT){const delimiter='runtime_'+randomUUID();await appendFile(process.env.GITHUB_OUTPUT,`files<<${delimiter}\n${files.join('\n')}\n${delimiter}\n`);}
