...
export const RUNTIME_CASE_IDS = Object.freeze(Array.from({length:16},(_,i)=>'runtime-'+String(i+1).padStart(2,'0')));
export const RUNTIME_SCREENSHOTS = Object.freeze(['runtime-01-1440.png','runtime-02-768.png','runtime-05-1440.png','runtime-16-320.png','runtime-16-768.png','runtime-16-1440.png']);
const fixed = () => { throw Error('RUNTIME_ARTIFACT_VALIDATION_FAILED'); };
...
export async function validateRuntimeArtifacts(workspace, runId) {
  ...
  let directory = root;
  for (const part of ['.cache','mode-runtime-artifacts',runId]) {
  ...
  if (new Set(s.screenshots).size !== s.screenshots.length || s.screenshots.some(name => typeof name !== 'string' || !RUNTIME_SCREENSHOTS.includes(name))) fixed();
