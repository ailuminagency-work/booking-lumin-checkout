...
export const DOCUMENT_CASE_IDS = Object.freeze(Array.from({length:8},(_,i)=>'document-'+String(i+1).padStart(2,'0')));
export const DOCUMENT_SCREENSHOTS = Object.freeze(['document-01-1440.png','document-02-768.png','document-03-1440.png','document-08-320.png','document-08-768.png','document-08-1440.png']);
const fixed = () => { throw Error('DOCUMENT_ARTIFACT_VALIDATION_FAILED'); };
...
export async function validateDocumentArtifacts(workspace, runId) {
  ...
  let directory = root;
  for (const part of ['.cache','mode-document-artifacts',runId]) {
  ...
  if (!keys(s,['schemaVersion','status','category','events','screenshots']) || s.schemaVersion !== 1 || !['passed','failed','timedout','interrupted'].includes(s.status) || !['COMPLETE','OUTPUT_SENTINEL_DETECTED','EVENT_BOUND','RUNNER_ERROR'].includes(s.category) || !Array.isArray(s.events) || s.events.length > 8 || !Array.isArray(s.screenshots) || s.screenshots.length > 6) fixed();
  for (const event of s.events) {
    if (!keys(event,['id','status','durationMs']) || !DOCUMENT_CASE_IDS.includes(event.id) || !['passed','failed','timedOut','skipped','interrupted'].includes(event.status) || !Number.isSafeInteger(event.durationMs) || event.durationMs < 0 || event.durationMs > 600000) fixed();
  }
  if (new Set(s.events.map(e=>e.id)).size !== s.events.length) fixed();
  if (s.status === 'passed' && (s.category !== 'COMPLETE' || s.events.length !== 8 || s.events.some(e=>e.status !== 'passed') || s.screenshots.length !== 6)) fixed();
  if (new Set(s.screenshots).size !== s.screenshots.length || s.screenshots.some(name => typeof name !== 'string' || !DOCUMENT_SCREENSHOTS.includes(name))) fixed();
