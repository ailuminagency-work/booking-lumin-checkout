...
const ids = new Set(Array.from({ length: 8 }, (_, i) => 'document-' + String(i + 1).padStart(2, '0')));
...
export function artifactDirectory(create = false) {
    const cwd = path.resolve(process.cwd()), real = fs.realpathSync(cwd), run = process.env.MODE_DOCUMENT_ARTIFACT_RUN_ID ?? '';
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(run))
        throw Error('ARTIFACT_RUN_REQUIRED');
    let current = cwd;
    for (const part of ['.cache', 'mode-document-artifacts', run]) {
...
export default class SafeReporter implements Reporter {
    ...
    private scan(chunk: string | Buffer) { const sentinel = process.env.MODE_DOCUMENT_SECRET_SENTINEL; if (sentinel && chunk.toString().includes(sentinel))
        this.unsafe = true; }
    onError(_error: TestError) { this.errors++; }
    onTestEnd(test: TestCase, result: TestResult) { if (this.events.length >= 8) {
        this.overflow = true;
        return;
    } const id = test.title.match(/^document-\d{2}(?=\s|$)/)?.[0] ?? 'unclassified'; const safeId = ids.has(id) ? id : 'unclassified'; const status = ['passed', 'failed', 'timedOut', 'skipped', 'interrupted'].includes(result.status) ? result.status : 'failed'; this.events.push({ id: safeId, status, durationMs: Math.max(0, Math.min(600000, Math.round(result.duration))) }); }
    async onEnd(result: FullResult) {
        ...
        for (const name of fs.readdirSync(root)) {
            ...
            if (stat.isSymbolicLink() || !stat.isFile() || stat.size > 5 * 1024 * 1024 || ++count > 6)
        ...
        if (result.status === 'passed' && (this.events.length !== 8 || new Set(this.events.map(e => e.id)).size !== 8 || this.events.some(e => !ids.has(e.id) || e.status !== 'passed') || approved.length !== 6)) this.overflow = true;
