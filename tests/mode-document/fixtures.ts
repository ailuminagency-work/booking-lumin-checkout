...
export function profile() { const a = addresses(); return { profileVersion: 'local-document-v1', rendererOrigin: a.renderer, apiOrigin: a.api, portalOrigin: a.portal, loaderUrl: a.renderer + '/assets/booking-lumin-loader.' + 'a'.repeat(64) + '.js' }; }
export function observerConfig(): PoolConfig {
  for (const name of ['LOCAL_HARNESS', 'FLOW_TEST_DISPOSABLE', 'MODE_INSTALLATIONS_TEST_DISPOSABLE', 'MODE_DOCUMENT_TEST_DISPOSABLE']) assert.equal(process.env[name], '1');
  ...
  assert.equal(process.env.PGUSER, 'postgres'); assert.match(process.env.PGDATABASE ?? '', /^lumin_mode_document_[a-z0-9_]+$/); assert.ok(process.env.PGDATABASE!.length <= 63);
...
export async function observer() {
  ...
  try {
    ...
    if (!rows.length) await db.query('insert into lumin.installation_profiles(version,renderer_origin,api_origin,portal_origin,loader_sha256) values($1,$2,$3,$4,$5)', [p.profileVersion, p.rendererOrigin, p.apiOrigin, p.portalOrigin, 'a'.repeat(64)]);
    const saved = (await db.query('select * from lumin.installation_profiles where version=$1', [p.profileVersion])).rows[0];
    assert.deepEqual([saved.renderer_origin, saved.api_origin, saved.portal_origin, saved.loader_sha256], [p.rendererOrigin, p.apiOrigin, p.portalOrigin, 'a'.repeat(64)]);
    return db;
...
export async function wire(base: string, target: string, tlsFixture: TlsFixture, options: { method?: string; headers?: Record<string, string>; rawHeaders?: string[]; signal?: AbortSignal; servername?: string; ca?: Buffer } = {}): Promise<Wire> {
  const u = new URL(base); assert.ok(Object.values(addresses()).includes(base)); assert.equal(u.protocol, 'https:');
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({ hostname: u.hostname, port: u.port, lookup, servername: options.servername ?? u.hostname, ca: options.ca ?? tlsFixture.cert, rejectUnauthorized: true, agent: false, path: target, method: options.method ?? 'GET', headers: options.rawHeaders ?? { Host: u.host, ...options.headers }, signal: options.signal }, res => {
      res.on('data', (chunk: Buffer) => { size += chunk.length; if (size > 65536) { req.destroy(); finish(Error()); } else chunks.push(chunk); });
    ...
    const timer = setTimeout(() => { req.destroy(); finish(Error()); }, 7000); req.on('error', error => finish(error)); req.end();
  });
}
...
const sockets = new WeakMap<https.Server, Set<import('node:stream').Duplex>>();
export async function listen(server: https.Server, origin: string, enforceSni = true) {
  ...
  server.on('secureConnection', socket => { if (enforceSni && socket.servername !== new URL(origin).hostname) socket.destroy(); });
...
export function closeServer(server: https.Server) { for (const socket of sockets.get(server) ?? []) socket.destroy(); server.closeAllConnections(); return new Promise<void>(resolve => server.close(() => resolve())); }
...
export async function startWorld(t: TlsFixture) {
  const a = addresses(); const p = profile(); const queries: string[] = []; const attempted = { reads: 0, issuance: 0 }; const owned: https.Server[] = [];
  ...
  const close = () => closeWithin([...(api ? [() => api!.close()] : []), ...(document ? [() => document!.close()] : []), ...(composition ? [() => composition!.close()] : []), ...owned.map(s => () => closeServer(s))]);
  try {
  composition = __createLocalModePolicyCompositionForTests({ profiles: [p] }, { env: { ...process.env }, poolFactory: (config: PoolConfig) => {
    const pool = new Pool(config); pool.on('error', () => {}); return { connect: async () => { const c = await pool.connect(); return { query: (sql: string, values?: unknown[]) => { if (queries.length >= 4096) throw Error('QUERY_OBSERVER_BOUND'); queries.push(sql); return c.query(sql, values); }, release: c.release.bind(c), on: c.on.bind(c), removeListener: c.removeListener.bind(c) }; }, end: () => pool.end(), on: pool.on.bind(pool), removeListener: pool.removeListener.bind(pool) };
  } });
  api = createModePolicyHttpHandler({ reader: composition.reader, profiles: [p] }); document = createModeDocumentHandler({ profiles: [p], fetch: policyFetch(t, attempted) });

    const A = https.createServer({ key: t.key, cert: t.cert, maxHeaderSize: 8192 }, (req, res) => { if (/session|submit|booking|request/.test(req.url ?? '')) attempted.issuance++; api!.handle(req, res); }); owned.push(A); await listen(A, a.api);
    ...
    for (const origin of [a.portal, a.merchant, a.second, a.forbidden]) {
      const server = https.createServer({ key: t.key, cert: t.cert }, (req, res) => {
        ...
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'Referrer-Policy': 'no-referrer' }); res.end('<!doctype html><title>Local parent</title><main><h1>Local parent fixture</h1><iframe title="Document frame" style="width:100%;height:600px" src="' + escaped + '"></iframe></main>');
      }); owned.push(server); await listen(server, origin);
    }
    const untrusted = https.createServer({ key: t.untrustedKey, cert: t.untrustedCert }, (_req, res) => { res.end('UNPINNED_KEY_MARKER'); }); owned.push(untrusted); await listen(untrusted, a.untrusted, false);
    return { addresses: a, queries, attempted, close };
