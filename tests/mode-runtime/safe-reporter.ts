...
export function artifactDirectory(create = false) {
    const cwd = path.resolve(process.cwd()), real = fs.realpathSync(cwd), run = process.env.MODE_RUNTIME_ARTIFACT_RUN_ID ?? '';
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(run))
        throw Error('ARTIFACT_RUN_REQUIRED');
    let current = cwd;
    for (const part of ['.cache', 'mode-runtime-artifacts', run]) {
...
export default class SafeReporter implements Reporter {
    ...
    private scan(chunk: string | Buffer) { const sentinel = process.env.MODE_RUNTIME_SECRET_SENTINEL; if (sentinel && chunk.toString().includes(sentinel))
        this.unsafe = true; }
