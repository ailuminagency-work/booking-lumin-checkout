...
create function pg_temp.reads(expected_tenant uuid,audit_count integer,is_platform boolean default false) returns void language plpgsql as $$
begin
 ...
 -- Hybrid RISK-4 (0013): a member sees only its own tenant's audit; a platform admin
 -- additionally sees platform-level (null-tenant) audit, never another tenant's.
 perform pg_temp.assert(not exists(
   select 1 from public.audit_events
   where not ( (expected_tenant is not null and tenant_id = expected_tenant)
               or (tenant_id is null and is_platform) )
 ),'foreign/platform audit');
end $$;
...
select pg_temp.reads(null,1,true);
select pg_temp.no_writes();
...
set local role authenticated;
select pg_temp.reads('a1800000-0000-4000-8000-000000000001',2,true);
select pg_temp.no_writes();
select pg_temp.aggregates(true);
reset role;
