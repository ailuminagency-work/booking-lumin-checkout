/**
 * webhookCrypto — dependency-free, cross-environment SHA-256 / HMAC-SHA256 for
 * the DEV mock payment provider's webhook signing.
 *
 * The mock runs in BOTH the browser checkout app and Node tests, so it cannot
 * use `node:crypto` (absent in the browser bundle) and must avoid the async
 * Web Crypto API to keep signing synchronous. This is a small, self-consistent
 * implementation used only to give the mock a keyed, forgery-resistant
 * signature; real providers verify with provider-native crypto (see
 * PaymentProviderContract). It is NOT audited production crypto.
 */
...
function sha256Bytes(msg: Uint8Array): Uint8Array {
  ...
  const withPad = new Uint8Array((((msg.length + 8) >> 6) + 1) * 64);
  withPad.set(msg);
  withPad[msg.length] = 0x80;
  // 64-bit length; high word is 0 for realistic payload sizes.
...
/** Random hex string (cross-env: Web Crypto where present, else a dev fallback). */
