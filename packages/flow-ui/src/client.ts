import {z} from 'zod';
import {Draft,FlowList,ServiceRender,PinnedRender,type FlowConfig,type Answers} from './types';
...
export function createFlowClient(base:string,localHarness=false,fetcher:typeof fetch=fetch){
 ...
 session:(installation:string)=>call('/api/installations/'+uuid(installation)+'/sessions',z.object({sessionToken:z.string().regex(/^[A-Za-z0-9_-]{43}$/),expiresAt:z.string().datetime({offset:true}),render:PinnedRender}).strict(),undefined,{}),
