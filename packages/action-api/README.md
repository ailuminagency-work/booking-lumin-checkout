...
Owner roster endpoints expose operational workers, crews/membership, eligibility and strictly typed shift transport. Provisioning is explicit; GET never writes. No worker login/access binding or job endpoint is exposed. The first Portal shift UI remains read-only. Snapshot bounds/Unicode/microsecond contracts are shared with @lumin/contracts; PostgreSQL retains timezone authority. Numeric SQL receipts are JSONB-projected and expected+1 checked inside the transaction before COMMIT. A lost response is uncertain; clients reconcile rather than automatically retrying with a newer version.

`npm run test:roster-http -w @lumin/action-api` requires the explicit disposable local environment and migrations through0023. It exercises real persistence/restart, complete scoped snapshot, CAS conflict, microseconds, Unicode, foreign identities and size rejection. Synthetic local Auth is not production verification or hosted deployment proof.
